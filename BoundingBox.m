%% BoundingBox
% Function to extract bounding box around characters
% Input: Image File, Parameters to improve detection
% Output: Features
% Note: Run from OCR_Extract_Features.m
% Note2: L is the connected components obtained from L=bwlabel(im2), where im2 is
% binarized
function [Features,cim,labelte,L,im2]=BoundingBox(L,im2,disp_on_off,AR_threshold,...
...
for i=1:Nc;
    ...
    if(minr~=maxr && minc~=maxc && abs((maxr-minr)-(maxc-minc))<AR_threshold && abs((maxr-minr)>R_Size) && abs((maxc-minc)>C_Size)&& abs((maxr-minr)/(maxc-minc))<AR )
        ...
        if(sum(cim(:))>Pixel_No)
            ...
            outtext=printlabels(temp);
            text(minc,minr,outtext,'Color',[1 1 1],'FontSize',16);
